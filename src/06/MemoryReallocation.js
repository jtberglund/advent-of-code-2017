...
const getRedistributionIndex = reduce((index, bank, i, arr) => (arr[index] < bank ? i : index), 0);

const incrementWrapping = R.curry((arrLength, index) => (index + 1) % arrLength);

// Redistributes the blocks in a memory bank to the other banks
const redistribute = (index, banks) => {
    let blocksToRedistribute = banks[index];
    const newBanks = Object.assign([], banks, { [index]: 0 });

    const increment = incrementWrapping(banks.length);
...
const countReallocationCycles = R.curry((numLoops, banks) => {
    const seenConfigurations = {};
    let cycles = 0;
    let redistributionIndex;
    while (!seenConfigurations[banks]) {
        redistributionIndex = getRedistributionIndex(banks);
        seenConfigurations[banks] = true;
        banks = redistribute(redistributionIndex, banks);
        cycles++;
    }
 ...
 */
const memoryReallocationPart1 = countReallocationCycles(1);
 ...
 */
const memoryReallocationPart2 = countReallocationCycles(2);

export { getRedistributionIndex, redistribute, memoryReallocationPart1, memoryReallocationPart2 };
